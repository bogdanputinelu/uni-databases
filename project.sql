...
--Sa se afiseze informatii (numele si tipul serviciului, numele partenerului, id_zoo, data in care s-a produs serviciul si cantitatea referitoare la serviciu) 
--despre tranzactiile produse intre parteneri si gradinile zoologice in care exista angajati care castiga salariul mediu 
--pe jobul lor in gradina zoologica in care ei lucreaza, care au numele de familie format din maxim 5 litere si care au fost angajati in 2020.
--rezultatele vor fi ordonate crescator dupa ziua din luna a datelor cand s-au produs tranzactiile
--41 rezultate

with salariu_mediu as(select round(avg(salariu)),id_zoo,id_job --se calculeaza salariul mediu pe fiecare job la fiecare gradina zoo
                      from angajati
                      group by id_zoo,id_job)
select p.nume_partener "Nume Partener",g.id_zoo "Gradina Zoo",s.nume_serviciu "Nume Serviciu", s.tip_serviciu "Tip Serviciu",
       b.cantitate "Cantitate Serviciu",to_char(data_serviciu,'DD-MM-YYYY') "Data Serviciului"
from partener p,serviciu s, gradina_zoologica g, beneficiaza b
where p.id_part=b.id_part         --se face join pe tabelele partener,serviciu,gradina_zoologica si beneficiaza
and s.id_serviciu=b.id_serviciu
and g.id_zoo=b.id_zoo
and g.id_zoo in (select id_zoo  --se selecteaza gradinile zoo care au cel putin un angajat care castiga salariul mediu pe jobul sau in aceeasi gradina,
                 from angajati  --care are numele format din maxim 5 litere si care a fost angajat in anul 2020
                 where (salariu,id_zoo,id_job) in (select * from salariu_mediu)
                 and length(nume)<=5
                 and to_char(data_ang,'YYYY')='2020')
order by to_char(data_serviciu,'DD');


--Cererea 2

--Se vrea marirea salariilor anumitor angajati. Sa se afiseze informatii despre acesti angajati (id_zoo,id_ang,id_job, prenumele si numele
--in aceeasi coloana, nr_tel,email,salariul_vechi, salariul_marit, data_ang), unde salariul_marit este salariul vechi crescut 
--cu 5% daca angajatul lucreaza ca dresor , cu 10% daca angajatul lucreaza ca veterinar, cu 15% daca lucreaza ca ingrijitor animale. 
--Daca angajatul nu are numar de telefon se va afisa "Nu exista numar de telefon". Angajatii carora li se va marii salariul sunt cei 
--care nu au fost angajati in ultima zi din luna, dar au fost angajati in ultimii 5 ani si care lucreaza la o gradina zoologica
--ce contine cel putin un animal carnivor caruia i-au fost impuse minim 2 restrictii.
--Rezultatele vor fii ordonate crescator dupa id_zoo.
--14 rezultate

select id_zoo "Gradina Zoologica",id_ang "Angajat",id_job "Job", prenume || ' ' || nume "Nume Complet", 
       nvl(nr_tel,'Nu exista numar de telefon') "Numar Telefon", email "Email",
       salariu "Salariu Vechi",case lower(id_job)   --calculam salariul pentru cele 3 job-uri
                               when 'drsr' then 
                                    salariu*1.05
                               when 'vtrnr' then
                                    salariu*1.1
                               when 'ingrj' then 
                                    salariu*1.15
                               end "Salariu Marit" ,data_ang "Data Angajare"
from angajati
where initcap(id_job) in ('Drsr','Vtrnr','Ingrj') --verificam ca job-ul angajatului sa fie unul din cele 3
and id_zoo in (select distinct id_zoo --furnizez gradinile zoologice care au cel putin un animal carnivor cu minim 2 restrictii
                 from animale
                 where id_animal in (select id_animal --verific daca animalul are minim 2 restrictii
                                     from restrictie
                                     group by id_animal
                                     having count(*)>=2)
                 and id_specie in (select id_specie --verific ca specia sa fie o specie carnivora
                                   from specie
                                   where lower(tip_dieta)='carnivor'))
and last_day(data_ang)!=data_ang
and months_between(sysdate,data_ang) < 60 --verific ca angajatul sa fie angajat in ultimii 5 ani
order by id_zoo;


--Cererea 3

--Sa se afiseze specia animalelor care, in ziua de 16 mai 2022, au fost hranite in afara intervalului orar 08:00 - 14:00
--cu un tip de hrana ce este valabil 2 saptamani de la cumparare, de catre un angajat care castiga salariul mediu al job-ului sau,
--adica media dintre salariul minim si maxim al job-ului. Se vor ordona rezultatele dupa numele speciei.
--2 rezultate

select s.nume_specie "Numele Speciei"
from specie s
where exists (select ani.id_specie
             from animale ani,hrana hr, angajati ang, hraneste hran,tip_hrana t
             where s.id_specie=ani.id_specie      
             and ani.id_animal=hran.id_animal
             and ang.id_ang=hran.id_ang
             and hr.id_hrana=hran.id_hrana
             and hr.id_tip_hrana=t.id_tip_hrana
             and data_hranire='16-MAY-22'
             and t.sapt_valabilitate=2
             and (to_char(hran.ora_hranire,'HH24:MI')>'14:00' or to_char(hran.ora_hranire,'HH24:MI')<'08:00')
             and ang.id_ang in (select id_ang    --angajatii care castiga salariul mediu de job
                                from angajati a
                                where salariu in(select (salariu_max+salariu_min)/2
                                                         from jobs
                                                         where a.id_job=id_job))) 
order by nume_specie;



--Cererea 4

--Sa se afiseze gradina zoologica, numele animalelor si daca acestea sunt batrane sau tinere (un animal este batran daca are varsta mai mare
--decat jumatatea varstei medie) pentru animalele carora nu le-au fost impuse restrictii si care fac parte din gradinile zoologice ale caror suma a preturilor produselor
--se afla peste media sumelor preturilor produselor din gradinile zoologice. Rezultatele vor fii ordonate descrescator dupa coloana in care apare batran/tanar.
--109 rezultate

with animale_fara_restr as (select id_animal  --animalele fara restrictii
                            from animale
                            minus
                            select unique id_animal
                            from restrictie)
select a.id_zoo "Gradina Zoologica",a.nume "Nume",aux.stadiu "Stadiu"
from animale a,(select decode(a.varsta,s.varsta_medie/2,'Tanar','Batran') stadiu,a.id_animal --stadiul tuturor animalelor
                from animale a,specie s
                where a.id_specie=s.id_specie) aux
where a.id_animal=aux.id_animal
and a.id_animal in (select * from animale_fara_restr)
and a.id_zoo in (select id_zoo --gradinile zoo care au suma preturilor mai mare decat media sumelor
                 from vand
                 having sum(cost) > (select avg(suma)
                                     from (select sum(cost) suma,id_zoo
                                           from vand
                                           group by id_zoo))
                 group by id_zoo)
order by aux.stadiu desc;



--Cererea 5

--Sa se afiseze informatiile angajatilor care au hranit in ziua de 16 mai 2022 animale care contin sirul 'an' in nume,
--ce nu lucreaza ca ingrijitori si care apartin gradinilor zoologice care au tranzactionat, in ultimii 5 ani, cu un partener
--al carei persoana de contact nu are numar de telefon.
--6 rezultate

select id_ang "ID Angajat", prenume || ' ' || nume "Nume Angajat", salariu "Salariu", id_zoo "Gradina Zoologica",
       id_job "Job", nr_tel "Numar Telefon", email "Email", data_ang "Data Angajarii"
from angajati
where id_ang in (select distinct id_ang --angajatii care au hranit animalele ce au sirul 'an' in nume, pe 16 mai 2022
                 from hraneste
                 where id_animal in (select distinct id_animal
                                     from animale
                                     where nume like '%an%')
                 and data_hranire='16-MAY-22')
and lower(id_job)!='ingrj'
and id_zoo in (select distinct gz.id_zoo     --gradinile zoologice ce au tranzactionat in ultimii 5 ani cu parteneri 
               from gradina_zoologica gz, beneficiaza b, partener p   --al caror persoana de contact nu are numar de telefon
               where gz.id_zoo=b.id_zoo
               and p.id_part=b.id_part
               and nr_tel_contact is null
...
--numarul animalelor este 7 ori mai mare ca numarul de ingrijitori
--19 rows updated

update angajati
set salariu=salariu*1.10
where lower(id_job)='ingrj'
and id_zoo in (select anim.id_zoo
               from (select count(*) nr_ang,id_zoo --numarul ingrijitorilor din gradina zoologica
                     from angajati
                     where lower(id_job)='ingrj'
                     group by id_zoo) ang,
                     (select count(*) nr_anim,id_zoo --numarul animalelor din gradina zoologica
                     from animale
                     group by id_zoo) anim
               where anim.id_zoo=ang.id_zoo
               and nr_anim>7*nr_ang);

rollback; --pentru a reveni la valorile de dinainte de actualizare

commit; --daca vrem sa permanentizam modificarile


--Actualizare
--Sa se mareasca salariile minime si maxime cu 20% pentru job-urile, mai putin ingrijitor, care s-au implicat in hranirea animalelor
--8 rows updated

update jobs j
set (salariu_min,salariu_max) = (select salariu_min*1.2,salariu_max*1.2
                                 from jobs
                                 where id_job=j.id_job)
where lower(id_job)!='ingrj'
and id_job in (select distinct id_job  --toate job-urile angajatilor care s-au implicat in hranirea animalelor
               from hraneste h,angajati a
               where a.id_ang=h.id_ang );
...
--Suprimare
--Sa se stearga toate restrictiile impuse asupra animalelor din speciile crepusculare
--21 rows updated

delete from restrictie
where id_animal in (select id_animal --animalele din speciile crepusculare
                    from animale a, specie s
                    where a.id_specie=s.id_specie
                    and lower(s.tip_animal)='crepuscular');
...
--le poate stabili. Acestea sunt toate serviciile de care nu a beneficiat inca de la anumiti parteneri
--de ex. daca gradina zoologica 1 nu a tranzactionat cu partenerul 120 pe 3 servicii, atunci gradina zoologica 1 are 3 posibilitati
--14 rezultate

select gz.id_zoo "Gradina Zoologica",count(*) "Posibilitati"
from gradina_zoologica gz,beneficiaza b,partener p,serviciu s
where gz.id_zoo=b.id_zoo(+)
and p.id_part=b.id_part(+) 
and s.id_serviciu=b.id_serviciu(+)
and b.id_part is null
and b.id_serviciu is null
group by gz.id_zoo;


--Cerere 1 Division
--Sa se afiseze produsele vandute de toate gradinile zoologice care au minim 25 de animale
--1 rezultat

select id_produs "ID Produs"
from vand
where id_zoo in(select id_zoo
                from animale
                group by id_zoo
                having count(*)>=25)
group by id_produs
having count(id_zoo)=(select count(count(*))
                      from animale
                      group by id_zoo
                      having count(*)>=25);

--Cerere 2 Division
--Sa se afiseze partenerii atasati tuturor gradinilor zoologice care au doar 2 angajati pe pozitia de administrator
--care castiga mai mult sau egal ca salariul mediu pe jobul de administrator.
--9 rezultate

with zoo_distincte as(select count(distinct id_zoo) nr,id_part --calculez pentru fiecare partener numarul
                     from beneficiaza                       --de gradini zoologice distincte care satisfac conditia
                     where id_zoo in (select id_zoo         --deoarece un partener poate tranzactiona cu aceeasi 
                                      from angajati         --gradina zoologica de mai multe ori
                                      where salariu >=(select (salariu_min+salariu_max)/2
                                                       from jobs
                                                       where lower(id_job)='admn')
                                      and lower(id_job)='admn'
                                      group by id_zoo
                                      having count(*)=2) 
                     group by id_part)

select id_part "ID Partener"
from beneficiaza b
where id_zoo in(select id_zoo
                from angajati
                where salariu >=(select (salariu_min+salariu_max)/2
...
having (select nr 
        from zoo_distincte 
        where id_part=b.id_part)=(select count(count(*))
                                  from angajati
                                  where salariu >=(select (salariu_min+salariu_max)/2
                                                   from jobs
                                                   where lower(id_job)='admn')
...
--7 rezultate
select nume_specie "Nume Specie"
from (select *
       from specie
       minus
       (select *
        from specie 
        where tip_animal='Crepuscular'
        union
        select s.*
        from specie s,animale a
        where s.id_specie=a.id_specie
        and a.nume='Ivy'))
where tip_dieta='Carnivor';
...
from specie
where tip_animal='Crepuscular'
and tip_dieta='Carnivor'
union
select sp.nume_specie
from (select nume_specie,id_specie 
     from specie
     where tip_dieta='Carnivor') sp, (select nume,id_specie
                                      from animale) an
where sp.id_specie=an.id_specie
and an.nume='Ivy');
